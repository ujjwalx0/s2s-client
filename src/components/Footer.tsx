...
const navItems = [
  ...
  { name: 'Privacy', href: '/privacy-policy' }, // for AdSense & Google News
  { name: 'Terms', href: '/terms-of-service' },  // for AdSense & Google News
];
    ...
    <footer className="bg-gray-900 text-gray-400 pt-10 pb-6 mt-20">
      <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6 sm:gap-12">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 text-sm sm:text-base">
          {navItems.map((item) => (
            <Link key={item.name} href={item.href} className="hover:text-white transition text-center sm:text-left">
              {item.name}
            </Link>
          ))}
        </div>
